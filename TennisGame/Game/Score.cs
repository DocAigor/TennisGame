...
namespace Game
{
    ...
    public class Score
    {
        ...
        public Tuple<string, string> GetResult(int pointA, int pointB)
        {            //caso Deuce e Advantage:
            ...
            if (pointA >= 3 && pointB >= 3)
            {
                ...
                else if (Math.Abs(pointA - pointB) == 1)
                {
                    ...
                    if (pointA > pointB)
                    {
                        return new Tuple<string, string>("Advantage", pointText[pointB]);
                    }
                    return new Tuple<string, string>(pointText[pointA], "Advantage");
                }
            }
            ...
            {
                if (pointA > pointB)
                    return new Tuple<string, string>("Winner", "Loser");
                return new Tuple<string, string>("Loser", "Winner");
            }

            //se non è nessuno dei casi precedenti allora è un punteggio base
            return new Tuple<string, string>(pointText[pointA], pointText[pointB]);
        }
